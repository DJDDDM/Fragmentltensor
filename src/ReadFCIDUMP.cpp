...
Htensor ReadFCIDUMP::getONE(){
	return ONE;
}

ERItensor ReadFCIDUMP::gettwo(){
	return two;
}
...
void ReadFCIDUMP::initarrays(){
	two = ERItensor(NORB,NORB,NORB,NORB);
	ONE = Htensor(NORB,NORB);
}
...
void ReadFCIDUMP::fillarrays(){
	...
	progressnextline();
	while (fourth != 0){
		two(first-1,second-1,third-1,fourth-1)=integral;
		two(second-1,first-1,third-1,fourth-1)=integral;
		two(second-1,first-1,fourth-1,third-1)=integral;
		two(first-1,second-1,fourth-1,third-1)=integral;

		two(third-1,fourth-1,first-1,second-1)=integral;
		two(fourth-1,third-1,first-1,second-1)=integral;
		two(fourth-1,third-1,second-1,first-1)=integral;
		two(third-1,fourth-1,second-1,first-1)=integral;

		progressnextline();
	}

	while(second != 0){
		ONE(first-1,second-1)=integral;
		ONE(second-1,first-1)=integral;
		progressnextline();
	}
	while(first != 0){
		progressnextline();
